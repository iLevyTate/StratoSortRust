...
impl DatabaseTestSuite {
    ...
    fn create_synthetic_test_files() -> Result<Vec<PathBuf>> {
        let mut files = Vec::new();
        let categories = vec![
            ("research", vec!["ai", "machine learning", "neural networks"]),
            ("finance", vec!["quarterly report", "financial statement", "budget"]),
            ("3d_models", vec!["3d print", "STL file", "CAD design"]),
            ("graphics", vec!["logo design", "brand identity", "vector graphics"]),
            ("documents", vec!["contract", "agreement", "proposal"]),
        ];
        
        for (category, _keywords) in categories {
            for i in 0..5 {
                files.push(PathBuf::from(format!("/synthetic/{}/file_{}.txt", category, i)));
            }
        }
    ...
    async fn test_database_initialization(&mut self) -> Result<()> {
        ...
        metrics.insert("vector_extension".to_string(), if vec_available { 1.0 } else { 0.0 });
    ...
    async fn test_embedding_storage(&mut self) -> Result<()> {
        ...
        let test_texts = vec![
            ("Machine learning research paper on neural networks", "research"),
        ...
        for (text, category) in test_texts {
            ...
            if embedding.len() != EMBEDDING_DIM {
                details.push_str(&format!("✗ Invalid embedding dimension: {} (expected {})\n", 
            ...
            match self.db.save_embedding(&path, &embedding).await {
                ...
                Err(e) => {
                    failed_saves += 1;
                    details.push_str(&format!("✗ Failed to store embedding for {}: {}\n", category, e));
                    passed = false;
                }
            }
        }
        ...
        metrics.insert("save_rate".to_string(), 
    ...
    async fn test_semantic_search(&mut self) -> Result<()> {
        ...
        let test_documents = vec![
            ("Deep learning with TensorFlow and PyTorch", "/docs/ml_framework.txt", "research"),
            ("Neural network architectures for NLP", "/docs/nlp_arch.txt", "research"),
            ("Annual financial report 2024", "/docs/finance_annual.txt", "finance"),
            ("Q4 earnings statement", "/docs/q4_earnings.txt", "finance"),
            ("3D printing tutorial for beginners", "/docs/3d_tutorial.txt", "3d_models"),
            ("STL file format specification", "/docs/stl_spec.txt", "3d_models"),
        ];
        ...
        let test_queries = vec![
            ("machine learning research", vec!["ml_framework", "nlp_arch"]),
                    ...
                    for (path, score) in &results {
                        for expected in &expected_matches {
                            if path.contains(expected) {
                                found_expected += 1;
                                details.push_str(&format!("✓ Found expected match: {} (score: {:.3})\n", 
                                    path, score));
                            }
                        }
                    }
        ...
        if accuracy < 0.7 {
            passed = false;
            details.push_str(&format!("✗ Search accuracy too low: {:.1}%\n", accuracy * 100.0));
        }
    ...
    async fn test_bulk_operations(&mut self) -> Result<()> {
        ...
        for i in 0..50 {
            analyses.push(FileAnalysis {
                ...
                category: if i % 3 == 0 { "research".to_string() } else if i % 3 == 1 { "finance".to_string() } else { "documents".to_string() },
        ...
        details.push_str(&format!("Bulk save: {} operations in {:.2}s ({:.1} ops/s)\n",
            successful_ops, bulk_duration.as_secs_f64(), ops_per_second));
        ...
        for i in 0..30 {
            let text = format!("Test document {} with some content for embedding generation", i);
            let embedding = generate_simple_embeddings(&text)?;
        ...
        details.push_str(&format!("Bulk embeddings: {} operations in {:.2}s ({:.1} emb/s)\n",
            embedding_count, embedding_duration.as_secs_f64(), embeddings_per_second));
    ...
    async fn test_concurrent_operations(&mut self) -> Result<()> {
        ...
        for handle in handles {
            match handle.await {
                Ok((save_ok, embed_ok)) => {
                    if save_ok { successful_saves += 1; }
                    if embed_ok { successful_embeddings += 1; }
                    if !save_ok || !embed_ok { errors += 1; }
                }
                Err(e) => {
                    errors += 1;
        ...
        metrics.insert("concurrent_embeddings".to_string(), successful_embeddings as f64);
        ...
        details.push_str(&format!("Concurrent operations: {} saves, {} embeddings, {} errors\n",
            successful_saves, successful_embeddings, errors));
        
        if success_rate < 0.95 {
            passed = false;
            details.push_str(&format!("✗ Success rate too low: {:.1}%\n", success_rate * 100.0));
        }
    ...
    async fn test_smart_folder_matching(&mut self) -> Result<()> {
        ...
        let test_files = vec![
            ("Deep learning paper on transformer architectures", "research", true, 0),
        ...
        for (content, category, should_match, expected_folder_idx) in test_files {
            ...
            if should_match {
                let folder_embedding = generate_simple_embeddings(
                    "semantic_match"
                )?;
                ...
                if matched {
                    correct_matches += 1;
                    details.push_str(&format!("✓ '{}' correctly matched to smart folder\n", content));
                } else {
                    details.push_str(&format!("✗ '{}' failed to match expected smart folder\n", content));
                }
            } else {
        ...
        if accuracy < 0.8 {
            passed = false;
            details.push_str(&format!("✗ Matching accuracy too low: {:.1}%\n", accuracy * 100.0));
        }
    ...
    async fn test_vector_extension(&mut self) -> Result<()> {
        ...
        match self.db.get_vector_stats().await {
            Ok(stats) => {
                details.push_str(&format!("Vector stats - Total: {}, Dimension: {}\n", 
                ...
                if stats.dimensions != EMBEDDING_DIM {
                    details.push_str(&format!("⚠ Unexpected vector dimension: {} (expected {})\n",
    ...
    async fn test_error_handling(&mut self) -> Result<()> {
        ...
        if concurrent_errors > 5 {
            details.push_str(&format!("⚠ High concurrent error rate: {}/10\n", concurrent_errors));
        } else {
            details.push_str(&format!("✓ Handled concurrent access well: {}/10 errors\n", concurrent_errors));
        }
    ...
    async fn test_performance_benchmarks(&mut self) -> Result<()> {
        ...
        metrics.insert("single_op_ms".to_string(), single_latency.as_millis() as f64);
        
        details.push_str(&format!("Single operation latency: {:.2}ms\n", single_latency.as_millis()));
        ...
        details.push_str(&format!("Vacuum operation: {:.2}ms\n", vacuum_duration.as_millis()));
    ...
    async fn test_data_integrity(&mut self) -> Result<()> {
            ...
            match self.db.get_analysis(&path).await? {
                Some(analysis) => {
                    if analysis.summary != expected_content {
                        ...
                        details.push_str(&format!("✗ Data mismatch for {}: expected '{}', got '{}'\n",
        ...
        if integrity_failures > 0 {
            passed = false;
            details.push_str(&format!("✗ {} integrity failures detected\n", integrity_failures));
        } else {
    ...
    async fn test_search_accuracy(&mut self) -> Result<()> {
        ...
        let documents = vec![
            // Research category
            ("Machine learning algorithms for natural language processing", "/search/ml_nlp.txt", "research"),
            ("Deep neural networks and computer vision applications", "/search/nn_cv.txt", "research"),
            ("Reinforcement learning in robotics", "/search/rl_robotics.txt", "research"),
            
            // Finance category
            ("Annual financial report fiscal year 2024", "/search/annual_2024.txt", "finance"),
            ("Quarterly earnings statement Q3", "/search/q3_earnings.txt", "finance"),
            ("Budget allocation and expense tracking", "/search/budget.txt", "finance"),
            
            // 3D Models category
            ("3D printing STL file for prototype design", "/search/stl_proto.txt", "3d_models"),
            ("CAD model for mechanical component", "/search/cad_mech.txt", "3d_models"),
            ("3D scanner output mesh data", "/search/scan_mesh.txt", "3d_models"),
            
            // Graphics category
            ("Company logo vector design SVG", "/search/logo_svg.txt", "graphics"),
            ("Brand identity guidelines document", "/search/brand_guide.txt", "graphics"),
            ("Marketing materials graphic assets", "/search/marketing_gfx.txt", "graphics"),
        ];
        ...
        let test_queries = vec![
            (
                ...
                vec!["/search/ml_nlp.txt", "/search/nn_cv.txt", "/search/rl_robotics.txt"],
                "AI Research Query"
            ),
            (
        ...
        for (query, expected_results, query_name) in test_queries {
            ...
            match self.db.semantic_search(&query_embedding, 5).await {
                Ok(results) => {
                    let result_paths: Vec<String> = results.iter().map(|(p, _)| p.clone()).collect();
                    ...
                    let precision = if !result_paths.is_empty() {
                        true_positives as f64 / result_paths.len().min(expected_results.len()) as f64
                    } else {
                        0.0
                    };
                    ...
                    details.push_str(&format!("{}: Precision={:.2}, Recall={:.2}\n", 
                        query_name, precision, recall));
                    
                    if recall >= 0.5 {
                        successful_queries += 1;
                        details.push_str(&format!("✓ {} passed (found {} of {} expected)\n",
                            query_name, true_positives, expected_results.len()));
                    } else {
                        details.push_str(&format!("✗ {} failed (found only {} of {} expected)\n",
                            query_name, true_positives, expected_results.len()));
                    }
                }
                Err(e) => {
        ...
        if f1_score < 0.6 {
            passed = false;
            details.push_str(&format!("✗ F1 score too low: {:.2} (threshold: 0.6)\n", f1_score));
        }
    ...
    async fn record_result(&self, result: TestResult) {
        ...
        println!("\n{} - {} ({:.2}s)", result.test_name, status, result.duration.as_secs_f64());
    ...
    async fn generate_test_report(&self, total_duration: Duration) -> Result<()> {
        ...
        for result in results.iter() {
            ...
            println!("  {} - {} ({:.2}s)", status, result.test_name, 
        ...
        for result in results.iter().filter(|r| !r.passed) {
            println!("  - Fix {}: Review test details for specific issues", result.test_name);
        }
        
        if results.iter().any(|r| r.test_name == "Vector Extension" && 
            r.metrics.get("vector_extension_available").is_some_and(|v| *v == 0.0)) {
            println!("  - Consider installing sqlite-vec extension for better performance");
        }
        
        if results.iter().any(|r| r.test_name == "Performance Benchmarks" && !r.passed) {
            println!("  - Optimize database queries and indexing for better performance");
        }
        
        if results.iter().any(|r| r.test_name == "Search Accuracy" && 
            r.metrics.get("f1_score").is_some_and(|v| *v < 0.7)) {

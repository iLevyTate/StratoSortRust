...
mod config_tests {
    ...
    async fn test_env_file_loading() {
        ...
        fs::write(&env_path, "OLLAMA_HOST=http://env-file-host:5678\nAI_PROVIDER=env-provider\n").unwrap();
...
mod ai_service_tests {
    ...
    async fn test_ollama_connection_retry() {
        ...
        assert!(ai_service.is_ok(), "AI service should initialize even if Ollama is offline (fallback mode)");
    ...
    async fn test_fallback_mode() {
        ...
        let analysis = service.analyze_file("test content about contracts", "text/plain").await;
...
mod database_tests {
    ...
    async fn test_clear_all_data() {
        ...
        assert!(analysis.unwrap().is_none(), "Should have no files after clear");
...
mod integration_tests {
    ...
    async fn test_full_workflow_with_fallback() {
        ...
        // Analyze a file
        let analysis = ai_service.analyze_file("Test document content", "txt").await.unwrap();

...
mod ollama_tests {
    ...
    async fn test_ollama_client_with_running_server() -> Result<()> {
        ...
        if embeddings.is_ok() {
            ...
            println!("Embeddings generation failed (model may not be installed): {:?}", embeddings);
        }
    ...
    async fn test_ollama_retry_mechanism() -> Result<()> {
        ...
        let analysis = client.analyze_file("Test document content", "text/plain").await;
        
        if analysis.is_ok() {
            let result = analysis.unwrap();
            assert!(!result.category.is_empty());
            println!("Analysis result: category={}, tags={:?}", result.category, result.tags);
        } else {
            println!("Analysis failed (model may not be installed): {:?}", analysis);
        }

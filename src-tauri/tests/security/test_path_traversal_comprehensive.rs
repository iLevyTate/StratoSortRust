    ...
    for i in 0..100 {
        ...
        if let Ok(path) = result {
            ...
            assert!(
                ...
                "Iteration {}: Path should stay within temp directory", i
            );
        }
    }
}
...
fn test_symlink_attack_prevention() {
    ...
    let symlink_patterns = vec![
        ...
        format!("{}\\..\\..\\windows\\system32\\config\\sam", temp_dir.path().display()),
    ];
        ...
        match result {
            Err(AppError::SecurityError { message }) => {
                assert!(
                    ...
                    "Should detect symlink attack pattern: {}", pattern
                );
            }
            Ok(sanitized_path) => {
                // If sanitization succeeds, ensure it doesn't access dangerous areas
                let path_str = sanitized_path.to_string_lossy();
                assert!(!path_str.contains("/etc/"), "Sanitized path should not access /etc/");
                assert!(!path_str.contains("\\system32\\"), "Sanitized path should not access system32");
                println!("Pattern '{}' sanitized to safe path: '{}'", pattern, path_str);
            }
            Err(e) => {
    ...
    for (expected, attack) in unicode_attacks {
        ...
        match result {
            Ok(path) => {
                ...
                assert!(!path_str.contains('\u{200b}'), "Zero-width space should be removed from: {}", attack);
                assert!(!path_str.contains('\u{200c}'), "Zero-width non-joiner should be removed from: {}", attack);
                assert!(!path_str.contains('\u{200d}'), "Zero-width joiner should be removed from: {}", attack);
                assert!(!path_str.contains('\u{202e}'), "Right-to-left override should be removed from: {}", attack);
                assert!(!path_str.contains('\u{200e}'), "Left-to-right mark should be removed from: {}", attack);
                
                println!("Unicode attack '{}' -> '{}' sanitized to: '{}'", expected, attack, path_str);
            }
            Err(e) => {
        ...
        match result {
            Err(AppError::SecurityError { message }) => {
                assert!(
                    ...
                    message.contains("not allowed"),
                    "Should detect canonicalization bypass: {}", bypass_attempt
                );
            }
            Ok(sanitized_path) => {
                ...
                assert!(
                    ...
                    canonical.display(), temp_canonical.display()
                );
                
                println!("Bypass attempt '{}' sanitized to safe path: '{}'", bypass_attempt, canonical.display());
            }
            Err(e) => {
                println!("Canonicalization bypass '{}' rejected: {:?}", bypass_attempt, e);
            }
        }
    }
}
...
fn test_directory_traversal_with_allowed_paths() {
    ...
    let test_cases = vec![
        ...
        (format!("{}/../restricted/secret.txt", allowed_dir.display()), false),
        (format!("{}/./../../restricted/secret.txt", allowed_dir.display()), false),
        (format!("{}/../../../etc/passwd", allowed_dir.display()), false),
    ];
            ...
            match result {
                ...
                Ok(sanitized_path) => {
                    ...
                    assert!(
                        ...
                        "Path '{}' should not access restricted areas, got: '{}'", test_path, path_str
                    );
                }
                Err(e) => {
...
fn test_file_extension_confusion_attacks() {
    ...
    for attack_filename in extension_attacks {
        ...
        match result {
            Ok(sanitized_path) => {
                let filename = sanitized_path.file_name()
                    .and_then(|n| n.to_str())
                    .unwrap_or("");
                ...
                assert!(!filename.contains('\u{200e}'), "Right-to-left mark should be removed");
                assert!(!filename.contains('\u{202e}'), "Right-to-left override should be removed");
                
                println!("Extension attack '{}' sanitized to: '{}'", attack_filename, filename);
            }
            Err(e) => {

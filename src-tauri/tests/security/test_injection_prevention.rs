    ...
    rt.block_on(async {
        ...
        for malicious_name in malicious_model_names {
            ...
            match result {
                ...
                Ok(_) => {
                    panic!("Malicious model name '{}' should have been rejected", malicious_name);
                }
                Err(e) => {
...
fn test_content_injection_prevention() {
    ...
    rt.block_on(async {
        ...
        for (content, mime_type) in malicious_content_tests {
            let result = analyze_with_ai(content.clone(), mime_type.to_string(), state_ref.clone()).await;
            ...
            match result {
                ...
                Ok(_) => {
                    panic!("Malicious content should have been rejected: content_len={}, mime='{}'", 
    ...
    rt.block_on(async {
        ...
        for (query, limit) in malicious_queries {
            ...
            match result {
                ...
                Ok(_) => {
                    panic!("Malicious search query should have been rejected: '{}'", query);
                }
                Err(e) => {
    ...
    for malicious_path in malicious_paths {
        ...
        match result {
            ...
            Ok(path) => {
                ...
                assert!(!path_str.contains(".."), "Path should not contain traversal: {}", path_str);
                assert!(!path_str.contains("/etc/"), "Path should not access system directories: {}", path_str);
                assert!(!path_str.contains("\\Windows\\System32\\"), "Path should not access system directories: {}", path_str);
...
fn test_filename_sanitization() {
    ...
    for (filename, should_remain_valid) in dangerous_filenames {
        ...
        if should_remain_valid {
            assert!(!sanitized.is_empty(), "Safe filename '{}' should not be completely removed", filename);
            assert!(!sanitized.contains(['<', '>', ':', '"', '|', '?', '*', '/', '\\', '\0']), 
                   "Sanitized filename should not contain dangerous characters: '{}'", sanitized);
        } else {
            // Dangerous filenames should either be completely sanitized or have dangerous parts removed
            assert!(!sanitized.contains(['<', '>', ':', '"', '|', '?', '*', '/', '\\', '\0']), 
                   "Sanitized filename should not contain dangerous characters: '{}' -> '{}'", filename, sanitized);
        }
    ...
    rt.block_on(async {
        ...
        for text in malicious_texts {
            ...
            match &text {
                ...
                t if t.len() > 100 * 1024 => {
                    assert!(matches!(result, Err(AppError::SecurityError { .. })), 
                ...
                t if t == "very normal text" => {
                    ...
                    match result {
                        ...
                        Err(e) => println!("Normal text rejected due to service unavailability: {:?}", e),
                    }
                }
                _ => {
                    ...
                    match result {
                        Ok(_) => {
                            ...
                            println!("Text '{}...' was processed (truncated for display)", &text[..text.len().min(50)]);
                        }
                        Err(e) => {
    ...
    for attack_path in unicode_attacks {
        ...
        match result {
            Ok(path) => {
                ...
                assert!(!path_str.contains('\u{202d}'), "Left-to-right override should be removed");
                assert!(!path_str.contains('\u{202e}'), "Right-to-left override should be removed");
                assert!(!path_str.contains('\u{200d}'), "Zero-width joiner should be handled");
                
                println!("Unicode path '{}' sanitized to: '{}'", attack_path, path_str);
            }
            Err(e) => {

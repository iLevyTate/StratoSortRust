...
use stratosort::utils::security::{validate_and_sanitize_path_legacy, is_path_allowed};
...
async fn test_scan_directory_path_traversal_prevention() {
    ...
    fs::write(safe_dir.join("allowed.txt"), "safe content").await.unwrap();
    fs::write(restricted_dir.join("secret.txt"), "restricted content").await.unwrap();
    ...
    for attack_path in traversal_attacks {
        println!("Testing scan_directory with path traversal: '{}'", attack_path);
        ...
        match result {
            Ok(files) => {
                ...
                for file in files {
                    assert!(!file.path.contains("restricted"),
                           "Restricted file accessed via path traversal: {}", file.path);
                    assert!(!file.path.contains("/etc/"),
                           "System file accessed via path traversal: {}", file.path);
                    assert!(!file.path.contains("\\system32\\"),
                           "System file accessed via path traversal: {}", file.path);
                    assert!(!file.path.contains("secret.txt"),
                           "Secret file accessed via path traversal: {}", file.path);
                }
                println!("Path traversal '{}' returned {} safe files", attack_path, files.len());
            }
            Err(AppError::SecurityError { message }) => {
                println!("Path traversal '{}' properly blocked: {}", attack_path, message);
...
async fn test_get_file_content_security_bypass_attempts() {
    ...
    let bypass_attempts = vec![
        ...
        // Encoding attacks
        format!("{}/%2e%2e/secret.txt", safe_file.parent().unwrap().display()),
        format!("{}\\%2e%2e\\secret.txt", safe_file.parent().unwrap().display()),
        ...
        // Unicode attacks
        format!("{}/..\u{2044}secret.txt", safe_file.parent().unwrap().display()), // Fraction slash
        format!("{}/..\u{002f}secret.txt", safe_file.parent().unwrap().display()), // Unicode slash
    ];

    for attack_path in bypass_attempts {
        println!("Testing get_file_content with bypass attempt: '{}'", attack_path);

        let state_clone = State::from(state.clone());
        let result = get_file_content(attack_path.clone(), Some("test_user".to_string()), state_clone, app.clone()).await;

        match result {
            Ok(content) => {
                // If content was retrieved, ensure it's not sensitive
                assert!(!content.contains("secret"),
                       "Secret content accessed via bypass: {}", attack_path);
                assert!(!content.contains("password"),
                       "Password content accessed via bypass: {}", attack_path);
                assert!(!content.contains("root:"),
                       "System file accessed via bypass: {}", attack_path);

                // Ensure content length is reasonable (not a device file)
                assert!(content.len() < 10_000_000,
                       "Suspiciously large content from bypass attempt: {} bytes", content.len());

                println!("Bypass attempt '{}' returned {} chars of safe content", attack_path, content.len());
            }
            Err(AppError::SecurityError { message }) => {
                println!("Bypass attempt '{}' properly blocked: {}", attack_path, message);
            }
            Err(AppError::FileNotFound { .. }) => {
...
async fn test_move_files_security_vulnerabilities() {
    ...
    fs::write(&important_file, "important content").await.unwrap();
    ...
    let malicious_move_ops = vec![
        ...
        MoveOperation {
            source: test_file.display().to_string(),
            destination: format!("{}\\..\\..\\Windows\\System32\\config\\SAM", target_dir.display()),
        },
        ...
        MoveOperation {
            source: test_file.display().to_string(),
            destination: format!("{}/..\u{002f}..\u{002f}restricted\u{002f}evil.txt", target_dir.display()),
        },
        ...
        Ok(results) => {
            ...
            for (i, result) in results.iter().enumerate() {
                ...
                if result.success {
                    ...
                    println!("Move {} succeeded: '{}' -> '{}'", i, result.source, result.destination);

                    // Check that destination is safe
                    assert!(!result.destination.contains("/etc/"),
                           "System directory written to: {}", result.destination);
                    assert!(!result.destination.contains("\\Windows\\System32\\"),
                           "System directory written to: {}", result.destination);
                    assert!(!result.destination.contains("/dev/"),
                           "Device file accessed: {}", result.destination);
                    ...
                    if Path::new(&result.destination).exists() {
                        ...
                        assert!(dest_path.starts_with(&temp_dir),
                               "File moved outside allowed directory: {}", result.destination);
                    }
                } else {
                    println!("Move {} blocked: {} - {}", i, result.source,
                           result.error.as_ref().unwrap_or(&"No error message".to_string()));
            ...
            assert!(!Path::new("/tmp/escaped.txt").exists(), "File escaped to /tmp");
            assert!(!Path::new("C:\\Windows\\System32\\evil.txt").exists(), "File escaped to System32");
            ...
            assert_eq!(important_content, "important content", "Important file was modified");
        }
        Err(e) => {
...
async fn test_analyze_files_input_validation() {
    ...
    fs::write(&safe_file, "safe content for analysis").await.unwrap();
    fs::write(&binary_file, b"\x00\x01\x02\x03\x04\x05\x06\x07").await.unwrap();
    ...
    let malicious_path_lists = vec![
        ...
        vec![
            ...
            format!("{}/../../../etc/passwd", safe_file.parent().unwrap().display()),
        ],
        ...
        vec![
            safe_file.display().to_string(),
            format!("{}/..\u{2044}restricted\u{2044}secret.txt", safe_file.parent().unwrap().display()),
        ],

        // Null byte injection
        vec![
            format!("{}\0", safe_file.display()),
            format!("{}/../restricted/secret.txt\0.txt", safe_file.parent().unwrap().display()),
        ],

        // Very long paths
        vec![
            format!("{}/{}", safe_file.parent().unwrap().display(), "A".repeat(10000)),
        ],
        ...
        match result {
            Ok(analyses) => {
                ...
                for analysis in analyses {
                    assert!(!analysis.path.contains("/etc/"),
                           "System file analyzed: {}", analysis.path);
                    assert!(!analysis.path.contains("\\Windows\\System32\\"),
                           "System file analyzed: {}", analysis.path);
                    assert!(!analysis.summary.contains("password"),
                           "Sensitive data in analysis summary: {}", analysis.summary);
                    assert!(!analysis.summary.contains("secret"),
                           "Sensitive data in analysis summary: {}", analysis.summary);

                    // Verify path is within temp directory
                    assert!(analysis.path.starts_with(&temp_dir.path().display().to_string()) ||
                           analysis.path == safe_file.display().to_string(),
                           "Analysis path outside safe directory: {}", analysis.path);
                }
            }
            Err(AppError::SecurityError { message }) => {
...
async fn test_process_dropped_paths_security() {
    ...
    let malicious_dropped_paths = vec![
        ...
        vec![
            safe_file.display().to_string(),
            format!("{}/..\u{2044}restricted\u{2044}secret.txt", safe_dir.display()),
    ...
    for (i, dropped_paths) in malicious_dropped_paths.iter().enumerate() {
        ...
        println!("  {} paths, first few: {:?}", dropped_paths.len(),
                dropped_paths.iter().take(3).collect::<Vec<_>>());

        let result = process_dropped_paths(dropped_paths.clone(), State(state.clone()), app.clone()).await;

        match result {
            Ok(processed) => {
                println!("Dropped paths #{} processed: {} valid files, {} valid folders, {} invalid",
                        i, processed.valid_files.len(), processed.valid_folders.len(), processed.invalid_paths.len());

                // Verify no sensitive files were processed
                for file in &processed.valid_files {
                    assert!(!file.path.contains("/etc/"),
                           "System file processed: {}", file.path);
                    assert!(!file.path.contains("\\Windows\\System32\\"),
                           "System file processed: {}", file.path);
                    assert!(!file.path.contains("secret"),
                           "Secret file processed: {}", file.path);
                    assert!(!file.path.contains("password"),
                           "Password file processed: {}", file.path);

                    // Verify file is within temp directory
                    assert!(file.path.starts_with(&temp_dir.path().display().to_string()),
                           "File outside safe directory processed: {}", file.path);
                }

                for folder in &processed.valid_folders {
                    assert!(!folder.path.contains("/etc/"),
                           "System folder processed: {}", folder.path);
                    assert!(!folder.path.contains("\\Windows\\System32\\"),
                           "System folder processed: {}", folder.path);

                    // Verify folder is within temp directory
                    assert!(folder.path.starts_with(&temp_dir.path().display().to_string()),
                           "Folder outside safe directory processed: {}", folder.path);
                }
                ...
                for invalid in &processed.invalid_paths {
                    ...
                    assert!(!invalid.reason.is_empty(),
                           "Invalid path should have reason: {}", invalid.path);
                }

                // Verify total size is reasonable
                assert!(processed.total_size < 10 * 1024 * 1024 * 1024,
                       "Total size too large: {} bytes", processed.total_size);
            }
            Err(AppError::SecurityError { message }) => {
...
async fn test_file_browse_dialog_security() {
        ...
        match result {
            Ok(paths) => {
                ...
                for path in paths {
                    ...
                    assert!(!path.contains("\\Windows\\System32\\"), "System path returned: {}", path);
    ...
    for (i, title) in malicious_titles.iter().enumerate() {
        println!("Testing browse_folder with malicious title #{}: '{}'", i, title);
        ...
        match result {
            Ok(folder_path) => {
                if !folder_path.is_empty() {
                    ...
                    assert!(folder_path.len() <= 4096, "Folder path too long: {} chars", folder_path.len());

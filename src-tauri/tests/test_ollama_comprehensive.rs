...
fn categorize_test_files() -> Vec<TestFileInfo> {
    let base_path = Path::new(
        r"C:\Users\benja\Documents\GitHub\StratoRust\src-tauri\tests\fixtures\data\sample_demo_files",
    );
...
async fn test_file_analysis(client: &OllamaClient, file_info: &TestFileInfo) -> TestResult {
    ...
    let (analysis_success, actual_category, confidence, tags, error) = match analysis_result {
        ...
        Err(e) => (false, String::new(), 0.0, vec![], Some(e.to_string())),
    };
...
async fn test_error_scenarios(client: &OllamaClient) -> Vec<String> {
    ...
    let result = client
        .analyze_file(&binary_str, "application/octet-stream")
        .await;
    match result {
    ...
    for i in 0..5 {
        let result = client
            .analyze_file(&format!("Test content {}", i), "text/plain")
            .await;
        match result {
            Ok(_) => {}
            Err(e) => {
...
async fn test_vision_models(client: &OllamaClient) -> Vec<TestResult> {
    ...
    let image_files = vec![
        TestFileInfo::new(
            PathBuf::from(
                r"C:\Users\benja\Documents\GitHub\StratoRust\src-tauri\tests\fixtures\data\sample_demo_files\20250906_1325_UFO Night Sky_remix_01k4g0x3pefeja3ye0v6j7es8v (1).png",
            ),
            "Logos/Graphic Art",
        ),
        TestFileInfo::new(
            PathBuf::from(
                r"C:\Users\benja\Documents\GitHub\StratoRust\src-tauri\tests\fixtures\data\sample_demo_files\20250911_1017_Imposter Financial Document_simple_compose_01k4wj305neqr9pjgx4m1b9mdr.png",
            ),
            "Finance",
        ),
    ];
    ...
    for file_info in image_files {
        ...
        let vision_available = client
            .list_models()
            .await
            .map(|models| {
                models
                    .iter()
                    .any(|m| m.contains("vision") || m.contains("llava"))
            })
            .unwrap_or(false);
...
async fn generate_test_report(
    ...
    let embedding_success = test_results
        .iter()
        .filter(|r| r.embedding_generated)
        .count() as f32
        / total as f32;

    let correct_categorizations = test_results
        .iter()
        .filter(|r| {
            r.analysis_success
                && r.actual_category
                    .to_lowercase()
                    .contains(&r.expected_category.to_lowercase())
        })
        .count() as f32;
    ...
    if categorization_accuracy < 0.8 {
        recommendations
            .push("Consider fine-tuning the model for better categorization accuracy".to_string());
    }

    if embedding_success < 0.9 {
        recommendations
            .push("Embedding generation needs improvement - check model availability".to_string());
    }

    if avg_processing_time > 5000 {
        recommendations
            .push("Processing time is high - consider optimization or caching".to_string());
    }
...
async fn test_ollama_comprehensive() -> Result<()> {
    ...
    for file in &test_files {
        *category_counts
            .entry(file.expected_category.clone())
            .or_insert(0) += 1;
    }
    ...
    for file_info in &test_files {
        ...
        println!(
            "  {} -> Expected: {}, Got: {}, Success: {}, Confidence: {:.2}",
            result.file_name,
    ...
    println!(
        "Embedding success rate: {:.1}%",
        report.embedding_success_rate * 100.0
    );
    println!(
        "Categorization accuracy: {:.1}%",
        report.categorization_accuracy * 100.0
    );
    println!(
    ...
    println!(
        ...
        report.performance_metrics.total_test_duration_ms
    );
    ...
    let report_path = Path::new(
        r"C:\Users\benja\Documents\GitHub\StratoRust\src-tauri\tests\ollama_test_report.json",
    );
    ...
    if test_passed {
        ...
        println!(
            "  - Categorization accuracy: >= 70% (got {:.1}%)",
            report.categorization_accuracy * 100.0
        );
        println!(
            "  - Embedding success rate: >= 50% (got {:.1}%)",
            report.embedding_success_rate * 100.0
        );
    }
...
async fn test_ollama_fallback_behavior() {
    ...
    match result {
        ...
        Err(e) => {
            ...
            assert!(e.to_string().contains("not running") || e.to_string().contains("unreachable"));
        }
    }
}
...
async fn test_smart_folder_matching_logic() {
    ...
    let _smart_folders = [
        ("Research", "Topics related to LLM, VLM and AI Research"),
        (
            "Logos/Graphic Art",
            "Any images that show a brand or logo like image",
        ),

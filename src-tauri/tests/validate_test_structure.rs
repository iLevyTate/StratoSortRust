...
mod validate_test_structure {
    ...
    const TEST_FIXTURES_PATH: &str = r"C:\Users\benja\Documents\GitHub\StratoRust\src-tauri\tests\fixtures\data";
    ...
    fn test_validate_fixture_structure() {
        ...
        if !fixtures_path.exists() {
            validation.add_issue(format!("Test fixtures directory does not exist: {}", TEST_FIXTURES_PATH));
        ...
        if !sample_files_path.exists() {
            validation.add_issue(format!("Sample files directory missing: {}", sample_files_path.display()));
        } else {
        ...
        if !smart_folders_path.exists() {
            validation.add_issue(format!("Smart folders directory missing: {}", smart_folders_path.display()));
        } else {
        ...
        assert!(validation.issues.is_empty(), "Test structure has critical issues");
    }
    ...
    fn validate_sample_files(path: &Path, validation: &mut TestStructureValidation) {
        ...
        if file_count < 10 {
            validation.add_warning(format!("Low number of sample files ({}), consider adding more for comprehensive testing", file_count));
        }

        // Check for diverse file types
        if file_types.len() < 5 {
            validation.add_warning(format!("Limited file type diversity ({} types), consider adding more file types", file_types.len()));
        }

        // Check for 3D print file types
        let has_3d_files = file_types.keys().any(|ext| 
            ["stl", "3mf", "obj", "gcode", "scad"].contains(&ext.as_str())
        );
        if !has_3d_files {
    ...
    fn validate_smart_folders(path: &Path, validation: &mut TestStructureValidation) {
        ...
        for (folder_name, display_name) in expected_folders {
            ...
            if !folder_path.exists() {
                validation.add_issue(format!("Missing smart folder: {} ({})", folder_name, display_name));
                ...
                match fs::read_dir(&folder_path) {
                    Ok(entries) => {
                        ...
                        if count > 0 {
                            validation.add_warning(format!("Smart folder '{}' contains {} items (should be empty for testing)", folder_name, count));
                        } else {
                            validation.add_info(format!("Smart folder '{}' is correctly empty", display_name));
                        }
                    }
                    Err(e) => {
                        validation.add_issue(format!("Cannot read smart folder {}: {}", folder_name, e));
                    }
                }
            }
        }
    ...
    fn validate_naming_conventions(path: &Path, validation: &mut TestStructureValidation) {
        ...
        for name in &folder_names {
            if !name.starts_with(expected_prefix) {
                validation.add_issue(format!("Folder '{}' doesn't follow naming convention (should start with '{}')", name, expected_prefix));
            }
        }
        ...
        for name in &folder_names {
            if name.contains('-') {
                validation.add_warning(format!("Folder '{}' uses hyphens instead of underscores", name));
            }
        }
    }

    fn test_file_categorization(sample_files_path: &Path, validation: &mut TestStructureValidation) {
        // Define expected categorizations
        let categorizations = vec![
            ("3D Print", vec![".stl", ".3mf", ".obj", ".gcode", ".scad"]),
            ("Finance", vec!["finance", "financial", "invoice", "expense"]),
        ...
        for (category, patterns) in categorizations {
            ...
            if found_files.is_empty() {
                ...
                validation.add_info(format!("Found {} files for '{}' category: {:?}", 
                    found_files.len(), category, found_files));
            }
        }
    }
    ...
    fn test_file_movement_simulation() {
        ...
        // Write test file
        fs::write(&test_file_path, "Test content for file movement").expect("Failed to create test file");
    ...
    fn test_reset_capability() {
        ...
        for folder_name in folders {
            ...
            if let Ok(entries) = fs::read_dir(&folder_path) {
                let count = entries.count();
                if count > 0 {
                    println!("⚠️ Smart folder '{}' is not empty ({} items)", folder_name, count);
                    all_empty = false;
                }
            }
        }

        assert!(all_empty, "Smart folders should be empty for test reusability");

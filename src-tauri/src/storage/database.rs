...
pub fn is_valid_sql_identifier(identifier: &str) -> bool {
    ...
    if !identifier.chars().next().unwrap_or(' ').is_ascii_alphabetic() && 
...
impl Database {
    pub async fn new(handle: &AppHandle) -> Result<Self> {
        ...
        if let Some(parent) = db_path.parent() {
            ...
            if let Err(e) = tokio::fs::write(&test_file, "test").await {
                ...
                return Err(AppError::DatabaseError {
                    message: format!("Database directory '{}' is not writable: {}. Please check permissions.", parent.display(), e),
                });
            }
            // Clean up test file
        ...
        let pool = SqlitePool::connect_with(
            connection_options
                .clone() // SQLite doesn't support traditional connection pooling like PostgreSQL
        )
        .await
        .map_err(|e| {
            ...
            match sqlx::query("SELECT 1").fetch_one(&pool).await {
                ...
                Err(e) => {
                    ...
                    if attempts >= max_attempts {
                        tracing::error!("Database connection test failed after {} attempts: {}", max_attempts, e);
                        return Err(AppError::DatabaseError {
                            message: format!("Database connection failed after {} attempts: {}. Database may be corrupted.", max_attempts, e),
                        });
                    }
                    tracing::warn!("Database connection attempt {} failed, retrying: {}", attempts, e);
    ...
    async fn initialize_schema(&self) -> Result<()> {
        ...
        self.create_index_safely("idx_analysis_category", "file_analysis", "category").await?;
        self.create_index_safely("idx_analysis_analyzed_at", "file_analysis", "analyzed_at").await?;
        self.create_index_safely("idx_operations_timestamp", "operations_history", "timestamp").await?;
        ...
        if self.vector_ext.is_available {
            ...
            if let Err(e) = self.vector_ext.create_vector_table(&self.pool, "vec_embeddings", self.vector_ext.get_dimensions()).await {
                warn!("Failed to create vector table: {}. Will use fallback storage.", e);
            }
        } else {
    ...
    #[allow(dead_code)]
    async fn create_index_safely(&self, index_name: &str, table_name: &str, column_name: &str) -> Result<()> {
        ...
        const ALLOWED_INDEXES: &[(&str, &str, &str)] = &[
            ...
            ("idx_smart_folders_created_at", "smart_folders", "created_at"),
            ("idx_smart_folders_updated_at", "smart_folders", "updated_at"),
        ];
        
        // Only allow predefined index combinations
        if !ALLOWED_INDEXES.iter().any(|(idx, tbl, col)| 
            idx == &index_name && tbl == &table_name && col == &column_name
        ) {
            return Err(AppError::SecurityError {
                message: format!("Unauthorized index configuration: {} on {}.{}", index_name, table_name, column_name),
            });
        }
        ...
        let query = format!("CREATE INDEX IF NOT EXISTS {} ON {}({})", index_name, table_name, column_name);
        ...
        match sqlx::query(&query).execute(&self.pool).await {
            ...
            Err(sqlx::Error::Database(db_err)) => {
                ...
                tracing::error!("Critical: Failed to create index {}: {}", index_name, db_err);
    ...
    async fn check_integrity(&self) -> Result<()> {
        ...
        if integrity_result != "ok" {
            ...
            match sqlx::query("PRAGMA quick_check").fetch_one(&self.pool).await {
                Ok(quick_result) => {
                    ...
                    if quick_check == "ok" {
                        ...
                        return Err(AppError::DatabaseError {
                            message: format!("Database corruption detected and cannot be recovered: {}", integrity_result),
                        });
                    }
                },
                Err(_) => {
    ...
    pub async fn save_embedding(&self, path: &str, embedding: &[f32], model_name: Option<&str>) -> Result<()> {
        ...
        if self.vector_ext.is_available {
            if let Err(e) = self.vector_ext.store_embedding(&self.pool, "vec_embeddings", path, embedding).await {
                warn!("Failed to store embedding in vector table: {}. Using fallback storage only.", e);
            }
        }
    ...
    pub async fn semantic_search(&self, query_embedding: &[f32], limit: usize) -> Result<Vec<(String, f32)>> {
        // Validate embedding dimensions
        if query_embedding.len() != self.vector_ext.embedding_dimensions {
            return Err(crate::error::AppError::InvalidInput { 
                message: format!("Embedding dimension mismatch: expected {}, got {}", 
                               self.vector_ext.embedding_dimensions, query_embedding.len()) 
            });
        }
        ...
        if self.vector_ext.is_available {
            ...
            match self.vector_ext.vector_search(&self.pool, "vec_embeddings", query_embedding, limit).await {
                Ok(results) => {
                    debug!("sqlite-vec search returned {} results", results.len());
                    return Ok(results);
                },
                Err(e) => {
                    warn!("sqlite-vec search failed, falling back to manual search: {}", e);
                }
            }
        }
        ...
        self.enhanced_cosine_similarity_search(query_embedding, limit).await
    }
    
    /// Enhanced cosine similarity search with better accuracy
    async fn enhanced_cosine_similarity_search(&self, query_embedding: &[f32], limit: usize) -> Result<Vec<(String, f32)>> {
            ...
            Err(_) => {
                ...
                match handle.path().app_local_data_dir() {
                    ...
                    Err(_) => {
                        ...
                        tracing::warn!("Unable to get app directories, falling back to current directory");
                        std::env::current_dir()
    ...
    async fn run_migrations(&self) -> Result<()> {
        ...
        if current_version == 0 {
            ...
            match self.initialize_schema_in_transaction(&mut tx).await {
                Ok(_) => {
                    sqlx::query("INSERT INTO schema_version (version) VALUES (?)").bind(1).execute(&mut *tx).await?;
                    tx.commit().await?;
        ...
        if current_version < 2 {
            ...
            match self.migrate_to_v2_in_transaction(&mut tx).await {
                Ok(_) => {
                    sqlx::query("INSERT INTO schema_version (version) VALUES (?)").bind(2).execute(&mut *tx).await?;
                    tx.commit().await?;
        ...
        if current_version < 3 {
            ...
            match self.migrate_to_v3_in_transaction(&mut tx).await {
                Ok(_) => {
                    sqlx::query("INSERT INTO schema_version (version) VALUES (?)").bind(3).execute(&mut *tx).await?;
                    tx.commit().await?;
    ...
    async fn initialize_schema_in_transaction(&self, tx: &mut sqlx::Transaction<'_, sqlx::Sqlite>) -> Result<()> {
        ...
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_analysis_category ON file_analysis(category)")
            .execute(&mut **tx)
            .await?;
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_analysis_analyzed_at ON file_analysis(analyzed_at)")
            .execute(&mut **tx)
            .await?;
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_operations_timestamp ON operations_history(timestamp)")
            .execute(&mut **tx)
            .await?;
    ...
    async fn migrate_to_v2_in_transaction(&self, _tx: &mut sqlx::Transaction<'_, sqlx::Sqlite>) -> Result<()> {
    ...
    async fn migrate_to_v3_in_transaction(&self, tx: &mut sqlx::Transaction<'_, sqlx::Sqlite>) -> Result<()> {
        ...
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_smart_folders_enabled ON smart_folders_v3(enabled)")
            .execute(&mut **tx)
            .await?;
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_embeddings_created ON embeddings_v3(created_at)")
            .execute(&mut **tx)
            .await?;
        ...
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_notifications_read ON notifications(read)")
            .execute(&mut **tx)
            .await?;
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_search_history_timestamp ON search_history(timestamp)")
            .execute(&mut **tx)
            .await?;
    ...
    async fn migrate_to_v3(&self) -> Result<()> {
        ...
        let indexes = [
            ...
            ("idx_smart_folders_created", "smart_folders_v3", "created_at"),
            ("idx_embeddings_created", "embeddings_v3", "created_at"),
            ("idx_operations_type", "operations_history_v3", "operation_type"),
            ("idx_operations_created", "operations_history_v3", "created_at"),
        ...
        for (index_name, table_name, column_name) in &indexes {
            ...
            if !is_valid_sql_identifier(index_name) || 
               !is_valid_sql_identifier(table_name) || 
               !is_valid_sql_identifier(column_name) {
                warn!("Skipping invalid SQL identifier: {}.{}.{}", table_name, column_name, index_name);
                continue;
            }
    ...
    pub async fn save_smart_folder(&self, folder: &crate::commands::organization::SmartFolder) -> Result<()> {
        let query = r#"
    ...
    pub async fn get_smart_folder(&self, id: &str) -> Result<Option<crate::commands::organization::SmartFolder>> {
    ...
    pub async fn list_smart_folders(&self) -> Result<Vec<crate::commands::organization::SmartFolder>> {
    ...
    pub async fn get_vector_stats(&self) -> Result<VectorStats> {
        self.vector_ext.get_vector_stats(&self.pool, "vec_embeddings").await
    }
    ...
    pub async fn maintain_vector_table(&self) -> Result<()> {
        if self.vector_ext.is_available {
            self.vector_ext.vacuum_vector_table(&self.pool, "vec_embeddings").await?;
        }
        Ok(())
    }
    
    // Notification-related methods
    pub async fn save_notification(&self, notification: &crate::commands::notifications::Notification) -> Result<()> {
        let query = r#"
        ...
        let metadata_json = notification.metadata.as_ref()
    ...
    pub async fn get_notifications(&self, limit: usize, unread_only: bool) -> Result<Vec<crate::commands::notifications::Notification>> {
    ...
    pub async fn save_search_history(&self, entry: &crate::commands::ai::SearchHistoryEntry) -> Result<()> {
        sqlx::query(
            r#"
    ...
    /// Get search history
    pub async fn get_search_history(&self, limit: usize) -> Result<Vec<crate::commands::ai::SearchHistoryEntry>> {
        let rows = sqlx::query(
            r#"

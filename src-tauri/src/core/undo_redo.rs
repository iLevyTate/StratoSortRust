use crate::{error::Result, storage::{Database, Operation}};
...
impl UndoRedoManager {
    ...
    pub fn with_limits(database: Arc<Database>, max_operations: usize, max_memory_mb: usize) -> Self {
        Self {
            database,
            undo_stack: Arc::new(RwLock::new(VecDeque::new())),
            redo_stack: Arc::new(RwLock::new(VecDeque::new())),
    ...
    fn estimate_operation_size(&self, operation: &Operation) -> usize {
        let mut size = operation.id.len() 
            + operation.operation_type.len() 
    ...
    async fn cleanup_if_memory_exceeded(&self) -> Result<()> {
        ...
        if memory_usage_mb > self.max_memory_mb {
            tracing::info!("Memory usage ({} MB) exceeds limit ({} MB), cleaning up old operations", 

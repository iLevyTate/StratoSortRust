            ...
            match exifreader.read_from_container(&mut bufreader) {
                Ok(exif) => {
                    ...
                    for field in exif.fields() {
                        match field.tag {
                            ...
                            Tag::DateTimeOriginal => {
                                if let Value::Ascii(ref vec) = field.value {
                                    if let Some(datetime) = vec.first() {
                                        metadata.datetime_original = Some(
                            ...
                            Tag::DateTimeDigitized => {
                                if let Value::Ascii(ref vec) = field.value {
                                    if let Some(datetime) = vec.first() {
                                        metadata.datetime_digitized = Some(
                                            String::from_utf8_lossy(datetime)
                                                .trim_matches('\0')
                                                .to_string(),
                                        );
                                ...
                                if let Value::Rational(ref rationals) = field.value {
                                    if let Some(exp_time) = rationals.first() {
                                        metadata.exposure_time =
                                            Some(format!("{}/{}", exp_time.num, exp_time.denom));
                                    }
                                }
                            }
                            Tag::Flash => {
                                ...
                                if let Value::Short(ref shorts) = field.value {
                                    if let Some(wb) = shorts.first() {
                                        metadata.white_balance =
                                            Some(self.decode_white_balance_value(*wb));
                                    }
                                }
                            }
                            Tag::MeteringMode => {
                                if let Value::Short(ref shorts) = field.value {
                                    if let Some(metering) = shorts.first() {
                                        metadata.metering_mode =
                                            Some(self.decode_metering_mode_value(*metering));
                                    }
                                }
                            }
                            Tag::ExposureMode => {
                                if let Value::Short(ref shorts) = field.value {
                                    if let Some(exp_mode) = shorts.first() {
                                        metadata.exposure_mode =
                                            Some(self.decode_exposure_mode_value(*exp_mode));
                                    }
                                }
                            }
                            Tag::SceneCaptureType => {
                                if let Value::Short(ref shorts) = field.value {
                                    if let Some(scene) = shorts.first() {
                                        metadata.scene_capture_type =
                                            Some(self.decode_scene_capture_type_value(*scene));
                                    }
                                }
                            }
                            _ => {}
                    ...
                    if let (Some(lat_ref), Some(lat), Some(lon_ref), Some(lon)) = (
                        ...
                        ) = (&lat_ref.value, &lat.value, &lon_ref.value, &lon.value)
                        {
                            if lat_vec.len() >= 3 && lon_vec.len() >= 3 {
                                ...
                                let lat_sign =
                                    if lat_ref_vec.first().map(|b| b[0] == b'S').unwrap_or(false) {
                                        -1.0
                                    } else {
                                        1.0
                                    };
                                let lon_sign =
                                    if lon_ref_vec.first().map(|b| b[0] == b'W').unwrap_or(false) {
                                        -1.0
                                    } else {
                                        1.0
                                    };
                    ...
                    if let (Some(alt_ref), Some(alt)) = (
                        ...
                        {
                            if let (Some(alt_ref_byte), Some(alt_rational)) =
                                (alt_ref_vec.first(), alt_vec.first())
                            {

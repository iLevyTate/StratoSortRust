...
async fn test_ai_service_initialization_with_ollama() {
    ...
    assert!(matches!(status.provider, AiProvider::Ollama) || matches!(status.provider, AiProvider::Fallback));
}
...
async fn test_update_config_provider_change() {
    ...
    assert!(matches!(new_status.provider, AiProvider::Ollama) || matches!(new_status.provider, AiProvider::Fallback));
}
...
async fn test_analyze_file_basic() {
    ...
    let result = ai_service.analyze_file(content, "text/plain").await.unwrap();
...
async fn test_analyze_file_with_path() {
    ...
    let result = ai_service.analyze_file_with_path(content, "application/pdf", path).await.unwrap();
    
    assert_eq!(result.path, path);
    assert!(!result.category.is_empty());
    assert!(!result.summary.is_empty());
    // Should detect invoice-related content
    assert!(result.tags.iter().any(|tag| tag.contains("invoice")) || 

...
pub async fn check_ollama_status(
    ...
    Ok(OllamaStatus {
        ...
        is_running,
        version: if is_running { Some("latest".to_string()) } else { None },
        models,
...
pub async fn analyze_with_ai(
    ...
    if !mime_type.chars().all(|c| c.is_alphanumeric() || c == '/' || c == '-' || c == '+' || c == '.') {
        return Err(crate::error::AppError::SecurityError {
    ...
    match state.ai_service.analyze_file(&content, &mime_type).await {
        ...
        Err(crate::error::AppError::AiError { .. }) => {
            ...
            Ok(crate::ai::FileAnalysis {
                ...
                detected_language: if mime_type.starts_with("text/") { Some("unknown".to_string()) } else { None },
...
pub async fn generate_embeddings(
    ...
    if text.len() > 100 * 1024 {  // 100KB limit for embeddings
        return Err(crate::error::AppError::SecurityError {
...
pub async fn semantic_search(
    ...
    match state.ai_service.generate_embeddings(&query).await {
        Ok(query_embedding) => {
            tracing::info!("Generated {} dimension embedding for query: '{}'", query_embedding.len(), query);
            
            let embedding_results = state.database.semantic_search(&query_embedding, limit * 2).await?;
            ...
            for (path, score) in embedding_results {
                ...
                if score > 0.3 {  // Raised from default to ensure quality
            ...
            tracing::info!("Found {} high-confidence embedding matches for query: '{}'", embedding_matches, query);
        }
        Err(e) => {
            tracing::warn!("Embedding generation failed for query '{}': {}. Falling back to text search.", query, e);
        }
    }
    ...
    let mut path_scores: std::collections::HashMap<String, (crate::ai::FileAnalysis, f32, Vec<String>)> = std::collections::HashMap::new();
    ...
    let mut hybrid_results: Vec<(crate::ai::FileAnalysis, f32, Vec<String>)> = path_scores.into_values().collect();
    ...
    let total_strategies: usize = hybrid_results.iter().map(|(_, _, strategies)| strategies.len()).sum();
    let multi_strategy_matches = hybrid_results.iter().filter(|(_, _, strategies)| strategies.len() > 1).count();
    
    tracing::info!(
        "Hybrid search for '{}': {} results, {} multi-strategy matches, {} total strategy hits", 
        query, hybrid_results.len(), multi_strategy_matches, total_strategies
    );
    
    // Extract just the analyses with metadata
    let enhanced_results: Vec<crate::ai::FileAnalysis> = hybrid_results.into_iter()
...
pub async fn quick_search(
    ...
    if let Some(ref types) = file_types {
        ...
        for file_type in types {
            if file_type.len() > 10 || !file_type.chars().all(|c| c.is_alphanumeric() || c == '.' || c == '-') {
                return Err(crate::error::AppError::SecurityError {
    ...
    let filename_results = state.database.search_by_filename(&query_lower, search_limit).await?;
    for path in filename_results {
        if let Ok(Some(analysis)) = state.database.get_analysis(&path).await {
            ...
            if let Some(ref types) = file_types {
                ...
                if !types.iter().any(|t| t.trim_start_matches('.').to_lowercase() == file_ext) {
                    continue;
                }
            }
    ...
    if results.len() < search_limit {
        let category_results = state.database.search_by_category(&query).await?;
        for path in category_results.into_iter().take(search_limit - results.len()) {
            if let Ok(Some(analysis)) = state.database.get_analysis(&path).await {
...
pub async fn advanced_search(
    ...
    if filters.use_semantic.unwrap_or(true) && filters.file_types.is_none() {
        if let Ok(embedding) = state.ai_service.generate_embeddings(&query).await {
            if let Ok(embedding_results) = state.database.semantic_search(&embedding, limit * 2).await {
                for (path, score) in embedding_results {
                    if let Ok(Some(analysis)) = state.database.get_analysis(&path).await {
    ...
    if let Some(ref category) = filters.category {
        ...
        for path in category_results {
            if let Ok(Some(analysis)) = state.database.get_analysis(&path).await {
                if !results.iter().any(|(a, _)| a.path == analysis.path) && apply_filters(&analysis, &filters).await? {
    ...
    let final_results: Vec<crate::ai::FileAnalysis> = results.into_iter().map(|(analysis, _)| analysis).collect();
...
fn infer_basic_category(mime_type: &str) -> String {
    match mime_type {
        ...
        t if t.contains("archive") || t.contains("zip") || t.contains("tar") => "Archive".to_string(),
...
async fn apply_filters(analysis: &crate::ai::FileAnalysis, filters: &SearchFilters) -> Result<bool> {
    // File type filter
    if let Some(ref file_types) = filters.file_types {
        ...
        if !file_types.iter().any(|t| t.trim_start_matches('.').to_lowercase() == file_ext) {
            return Ok(false);
        }
    }
    ...
    // Category filter
    if let Some(ref category) = filters.category {
        if !analysis.category.to_lowercase().contains(&category.to_lowercase()) {
            return Ok(false);
        }
    }
...
pub async fn suggest_organization(
    ...
    state.ai_service.suggest_organization(files, smart_folders).await
}
            ...
            Ok(content) => {
                ...
                match state.ai_service.analyze_file(&content, &mime_type).await {
                    Ok(mut analysis) => {
                        ...
                        if let Ok(embedding) = state.ai_service.generate_embeddings(&format!("{} {}", analysis.summary, analysis.tags.join(" "))).await {
                            let model_name = state.config.read().ollama_embedding_model.clone();
                            let _ = state.database.save_embedding(&analysis.path, &embedding, Some(&model_name)).await;
                        }

...
pub async fn update_smart_folder(
    ...
    let mut smart_folder = state.database.get_smart_folder(&id).await?
...
pub async fn apply_smart_folder_rules(
    ...
    let smart_folder = state.database.get_smart_folder(&folder_id).await?
        .ok_or_else(|| crate::error::AppError::NotFound {
                ...
                if !dry_run {
                    match rule.action.action_type {
                        ...
                        ActionType::Rename => {
                            ...
                            let parent = std::path::Path::new(&file_path).parent().unwrap_or(std::path::Path::new("."));
                        ...
                        ActionType::Tag => {
                            ...
                            tracing::debug!("Tagged file {} with folder {}", file_path, rule.action.target_folder);
                        }
                    }
                }
...
pub async fn auto_organize_directory(
    ...
    state.update_progress(operation_id, 0.1, "Scanning directory for files".to_string());
    ...
    let ai_analyses = if use_ai {
        state.update_progress(operation_id, 0.3, "Starting AI analysis of files".to_string());
        ...
        for (index, file_path) in file_paths.iter().enumerate() {
            ...
            if state.database.get_analysis(file_path).await.unwrap_or(None).is_none() {
    ...
    if use_ai && all_previews.is_empty() {
        state.update_progress(operation_id, 0.9, "Generating AI organization suggestions".to_string());
        
        let smart_folders = state.database.list_smart_folders().await.unwrap_or_default();
        let suggestions = state.ai_service.suggest_organization(file_paths, smart_folders).await?;
    ...
    state.update_progress(
        operation_id, 
        1.0, 
        format!("Organization complete: {} suggestions generated", all_previews.len())
    );
    state.complete_operation(operation_id);
...
pub async fn suggest_file_organization(
    ...
    let smart_folders = state.database.list_smart_folders().await.unwrap_or_default();
    state.ai_service.suggest_organization(paths, smart_folders).await
}
...
pub async fn apply_organization(
    ...
    for (index, op) in operations.iter().enumerate() {
        ...
        state.update_progress(
            operation_id,
            progress,
            format!("Organizing: {}", std::path::Path::new(&op.source_path).file_name().unwrap_or_default().to_string_lossy())
        );
        
        let result = match perform_organization_operation(op).await {
            Ok(_) => {
                // Record for undo
                if let Err(e) = state.undo_redo.record_move(&op.source_path, &op.target_path).await {
                    tracing::warn!("Failed to record undo operation: {}", e);
                }
                
                // Emit success event for this operation
                let _ = app.emit("organization-success", serde_json::json!({
            ...
            Err(e) => {
                tracing::error!("Organization operation failed for {}: {}", op.source_path, e);
                
                // Emit error event for this operation
                let _ = app.emit("organization-failed", serde_json::json!({
                    "source_path": op.source_path,
                    "target_path": op.target_path,
                    "action": op.action,
                    "error": e.to_string(),
...
async fn apply_smart_folder_rules_enhanced(
    ...
    let folder = state.database.get_smart_folder(&folder_id).await?
        .ok_or(crate::error::AppError::DatabaseError {
    ...
    for file_path in file_paths {
        ...
        let confidence = calculate_folder_match_confidence_with_ai(&file_path, &folder, ai_analyses).await;
        
        if confidence > 0.5 {
            let target_path = std::path::Path::new(&folder.target_path)
                .join(std::path::Path::new(&file_path).file_name().unwrap_or_default())
                .display()
                .to_string();
...
async fn calculate_folder_match_confidence_with_ai(
    ...
    if let Some(ai_category) = ai_analyses.get(file_path) {
        ...
        if folder_name_lower.contains(&category_lower) || 
           category_lower.contains(&folder_name_lower) ||
           folder.description.as_ref().map(|d| d.to_lowercase().contains(&category_lower)).unwrap_or(false) {
...
fn infer_file_category(file_path: &str) -> Option<String> {
    ...
    match extension.to_lowercase().as_str() {
        // Document files
        "pdf" | "doc" | "docx" | "docm" | "dot" | "dotx" | "dotm" | "txt" | "rtf" | "odt" | "ott" | "pages" => Some("Documents".to_string()),
        
        // Image files
        "jpg" | "jpeg" | "png" | "gif" | "bmp" | "svg" | "webp" | "tiff" => Some("Images".to_string()),
        
        // Video files
        "mp4" | "avi" | "mkv" | "mov" | "wmv" | "flv" | "webm" | "m4v" => Some("Videos".to_string()),
        
        // Audio files
        "mp3" | "wav" | "flac" | "aac" | "ogg" | "wma" | "m4a" => Some("Audio".to_string()),
        
        // Code files
        "rs" | "py" | "js" | "ts" | "java" | "c" | "cpp" | "h" | "hpp" | "go" | "php" => Some("Code".to_string()),
        
        // Archive files
        "zip" | "rar" | "7z" | "tar" | "gz" | "bz2" => Some("Archives".to_string()),
        
        // Spreadsheet files
        "xls" | "xlsx" | "xlsm" | "xlsb" | "xlt" | "xltx" | "xltm" | "csv" | "tsv" | "ods" | "ots" | "numbers" => Some("Spreadsheets".to_string()),
        
        // Presentation files
        "ppt" | "pptx" | "pptm" | "ppsx" | "pps" | "pot" | "potx" | "potm" | "odp" | "key" => Some("Presentations".to_string()),
        
        // 3D Print files
        "stl" | "obj" | "3mf" | "amf" | "ply" | "x3d" | "dae" | "blend" | "fbx" | "3ds" | "max" | "c4d" | "ma" | "mb" | "skp" | "dwg" | "dxf" | "step" | "stp" | "iges" | "igs" | "brep" | "gcode" | "g" | "ngc" | "cnc" | "prusa" | "chitubox" | "lgs" | "pws" | "sl1" | "ctb" | "cbddlp" | "photon" | "pmsq" | "zip3d" => Some("3D Print Files".to_string()),
        ...
        _ => {
            ...
            } else if filename_lower.contains("print") || filename_lower.contains("model") || filename_lower.contains("miniature") || filename_lower.contains("figurine") || filename_lower.contains("prototype") {
...
pub async fn validate_rule(
    ...
    match rule.rule_type {
        RuleType::FileSize => {
            if !matches!(rule.condition.operator, ConditionOperator::GreaterThan | ConditionOperator::LessThan | ConditionOperator::Equals) {
            ...
            if rule.condition.value.parse::<u64>().is_err() {
                errors.push("File size condition value must be a valid number (in bytes)".to_string());
            } else {
        ...
        RuleType::CreationDate | RuleType::ModificationDate => {
            ...
            if rule.condition.value.parse::<u64>().is_err() {
                ...
                if chrono::DateTime::parse_from_rfc3339(&rule.condition.value).is_err() {
                    errors.push("Date condition value must be a valid timestamp or ISO 8601 date string".to_string());
                }
            }
        }
        RuleType::FileExtension => {
        ...
        RuleType::FileName | RuleType::Path => {
            if matches!(rule.condition.operator, ConditionOperator::GreaterThan | ConditionOperator::LessThan) {
                warnings.push("Numeric comparison operators are unusual for file name or path rules".to_string());
            }
            ...
            if matches!(rule.condition.operator, ConditionOperator::Regex) {
                if let Err(e) = regex::Regex::new(&rule.condition.value) {
                    ...
                    suggestions.push("Test your regex pattern with sample data to ensure it works as expected".to_string());
                }
            }
        }
        RuleType::MimeType => {
            ...
            if !rule.condition.value.contains('/') {
                warnings.push("MIME types typically contain a '/' character (e.g., 'image/jpeg')".to_string());
            }
        }
        RuleType::FileContent => {
            if matches!(rule.condition.operator, ConditionOperator::GreaterThan | ConditionOperator::LessThan) {
                warnings.push("Numeric comparison operators are unusual for file content rules".to_string());
            }
    ...
    // Validate priority
    if rule.priority < 0 {
        warnings.push("Negative priority values are unusual. Lower numbers = higher priority".to_string());
    }
    ...
    if rule.condition.case_sensitive.is_none() {
        suggestions.push("Consider specifying case_sensitive explicitly for predictable behavior".to_string());
    }
    
    // Action validation
    match rule.action.action_type {
        ActionType::Rename => {
            if rule.action.rename_pattern.is_none() || rule.action.rename_pattern.as_ref().map_or(true, |p| p.trim().is_empty()) {
...
async fn test_rule_against_single_file(
    ...
    let (matches, match_value) = match rule.rule_type {
        ...
        RuleType::Path => {
            (evaluate_condition(&rule.condition, file_path), file_path.to_string())
        }
        RuleType::MimeType => {
            let mime = mime_guess::from_path(path)
                .first_or_octet_stream()
        ...
        RuleType::FileContent => {
            ...
            (evaluate_condition(&rule.condition, preview), format!("Content preview ({} chars)", preview.len()))
        }
        RuleType::CreationDate => {
            let metadata = tokio::fs::metadata(file_path).await?;
            ...
            let timestamp_str = duration.as_secs().to_string();
            (evaluate_condition(&rule.condition, &timestamp_str), timestamp_str)
        }
        RuleType::ModificationDate => {
            let metadata = tokio::fs::metadata(file_path).await?;

use crate::{
    ...
    error::Result,
    state::AppState,
    commands::organization::SmartFolder,
};
...
pub async fn complete_first_run_setup(
    ...
    let mut config = Config::create_first_run_config(&app_handle, setup.smart_folder_location.unwrap_or_default())?;
    ...
    *state.config.write() = config.clone();
    ...
    let _ = app_handle.emit("setup-complete", serde_json::json!({
        "success": true,
...
#[tauri::command]
pub async fn reset_to_first_run(
    app_handle: AppHandle,
    state: State<'_, AppState>,
) -> Result<()> {
...
async fn create_default_smart_folders(state: &AppState) -> Result<()> {
    ...
    for folder in default_folders {
        if let Err(e) = state.database.save_smart_folder(&folder).await {
            tracing::warn!("Failed to create default smart folder '{}': {}", folder.name, e);
        } else {

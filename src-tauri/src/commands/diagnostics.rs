...
async fn check_file_permissions(config: &crate::config::Config, _app: &AppHandle) -> Vec<PathPermissionCheck> {
    let mut checks = Vec::new();
    
    // Check important paths from config
    let paths_to_check = vec![
        &config.default_smart_folder_location,
    ];
...
fn generate_recommendations(
    ...
    if !config_errors.is_empty() {
        recommendations.push("Fix configuration errors to ensure proper application behavior".to_string());
    }
    
    // AI service recommendations
    if !ai_status.is_available {
        if ai_status.provider == "Ollama" {
            recommendations.push("Install and start Ollama service for AI-powered file analysis".to_string());
            recommendations.push("Check that Ollama is running on the configured host".to_string());
        } else {
            recommendations.push("AI service is not available - check your AI provider configuration".to_string());
        }
    }
    
    if ai_status.models_available.is_empty() && ai_status.is_available {
        recommendations.push("No AI models available - install required models in your AI service".to_string());
    }
    
    // Database recommendations
    if !db_status.connection_status.contains("Connected") {
        recommendations.push("Database connection issues detected - check database configuration".to_string());
    }
    ...
    if resources.memory_limit_reached {
        recommendations.push("High memory usage detected - consider reducing cache size or concurrent operations".to_string());
    }
    
    if resources.disk_space_low {
        recommendations.push("Low disk space - free up space or move application data to another drive".to_string());
    }
    
    if resources.cpu_usage_percent > 80.0 {
        recommendations.push("High CPU usage - consider reducing concurrent operations".to_string());
    }

...
impl Config {
    ...
    fn apply_env_overrides(&mut self) {
        ...
        if let Ok(max_analysis) = std::env::var("MAX_CONCURRENT_ANALYSIS") {
            self.max_concurrent_analysis = max_analysis.parse().unwrap_or(self.max_concurrent_analysis);
        }
        if let Ok(max_ops) = std::env::var("MAX_CONCURRENT_OPERATIONS") {
            self.max_concurrent_operations = max_ops.parse().unwrap_or(self.max_concurrent_operations);
        }
        ...
        if let Ok(show_notifications) = std::env::var("SHOW_NOTIFICATIONS") {
            self.show_notifications = show_notifications.parse().unwrap_or(self.show_notifications);
        }
        ...
        if let Ok(enable_crash_reports) = std::env::var("ENABLE_CRASH_REPORTS") {
            self.enable_crash_reports = enable_crash_reports.parse().unwrap_or(self.enable_crash_reports);
        }
        ...
        if let Ok(confirm_delete) = std::env::var("CONFIRM_BEFORE_DELETE") {
            self.confirm_before_delete = confirm_delete.parse().unwrap_or(self.confirm_before_delete);
        }
    ...
    fn validate_ai_settings(&self, errors: &mut Vec<String>) {
        ...
        if !self.ai_provider.is_empty() && !["ollama", "openai", "anthropic"].contains(&self.ai_provider.as_str()) {
        ...
        } else if !self.ollama_host.starts_with("http://") && !self.ollama_host.starts_with("https://") {
    ...
    fn validate_file_settings(&self, errors: &mut Vec<String>) {
        ...
        if self.max_single_file_size_mb > 1024 { // 1GB in MB
    ...
    fn validate_performance_settings(&self, errors: &mut Vec<String>) {
        ...
        if self.cache_size > 10 * 1024 * 1024 * 1024 { // 10GB
        ...
        if self.max_total_memory_mb > 32 * 1024 { // 32GB in MB
    ...
    fn validate_ui_settings(&self, errors: &mut Vec<String>) {
        if !["light", "dark", "auto"].contains(&self.theme.as_str()) {
            errors.push(format!("Invalid theme: {}. Must be light, dark, or auto", self.theme));
        }
    ...
    fn validate_advanced_settings(&self, errors: &mut Vec<String>) {
        if !["error", "warn", "info", "debug", "trace"].contains(&self.log_level.as_str()) {
            errors.push(format!("Invalid log level: {}. Must be error, warn, info, debug, or trace", self.log_level));
        }
        ...
        if self.history_retention > 365 * 24 * 60 * 60 { // 1 year in seconds
    ...
    fn config_path(handle: &AppHandle) -> Result<PathBuf> {
        let app_dir = handle.path().app_config_dir()
    ...
    fn default_smart_folder_path(handle: &AppHandle) -> Result<String> {
        let documents_dir = handle.path().document_dir()
            .map_err(|e| crate::error::AppError::ConfigError {
    ...
    pub fn get_warnings(&self) -> Vec<String> {
        ...
        if self.max_file_size > 1024 * 1024 * 1024 { // 1GB
            warnings.push("max_file_size is very large (>1GB) - this may cause memory issues".to_string());
        }
        
        if self.cache_size > 1024 * 1024 * 1024 { // 1GB
            warnings.push("cache_size is very large (>1GB) - this may cause memory issues".to_string());
        }
        
        if self.max_concurrent_analysis > 10 {
            warnings.push("max_concurrent_analysis is very high (>10) - this may overwhelm your system".to_string());
        }
        
        if self.history_retention > 365 * 24 * 60 * 60 { // 1 year in seconds
            warnings.push("history_retention is very long (>1 year) - this may cause database bloat".to_string());
        }
        
        if self.undo_history_size > 10000 {
            warnings.push("undo_history_size is very large (>10000) - this may cause memory issues".to_string());
        }
        
        if !self.default_smart_folder_location.starts_with('/') && !self.default_smart_folder_location.contains(':') {
